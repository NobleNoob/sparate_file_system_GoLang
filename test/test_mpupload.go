...
func signin(username, password string) (token string, err error) {
	resp, err := http.PostForm(
		"http://localhost:8880/user/signin",
		url.Values{
			"username": {username},
			"password": {password},
		})
...
func main() {
	...
	password := "admin"
	// 需要上传的文件名及文件hash
	fpath := "/Users/zhouliren/Downloads/Course-Slides.pdf"
	...
	// 1. 请求初始化分块上传接口
	resp, err := http.PostForm(
		"http://localhost:8880/file/mpupload/init",
		url.Values{
			"username": {username},
			"token":    {token},
			"filehash": {fhash},
	...
	// 3. 请求分块上传接口
	tURL := "http://localhost:8880/file/mpupload/uppart?" +
		"username=admin&token=" + token + "&uploadid=" + uploadID
	multipartUpload(fpath, tURL, chunkSize)

	// 4. 请求分块完成接口
	//resp, err = http.PostForm(
	//	"http://localhost:8880/file/mpupload/complete",
	//	url.Values{
	//		"username": {username},
	//		"token":    {token},
	//		"filehash": {fhash},
